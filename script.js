...
const PDF_SYSTEM_PROMPT = `Eres un técnico senior de electrónica. A partir del historial de chat entre un usuario y el asistente de diagnóstico, genera un REPORTE DE DIAGNÓSTICO profesional en formato Markdown. El reporte debe incluir:
- Título con el nombre del equipo y la falla
...
async function doGenerateImage() {
  ...
  try {
    // Usar Puter.js con GPT Image 2
    const result = await puter.ai.txt2img(prompt, { model: 'openai/gpt-image-2', quality: 'high' });

    imageLoading.style.display = 'none';
    imageGenerateBtn.disabled = false;

    // Result es un elemento <img> con src de data URL
    const imageUrl = result.src || result.getAttribute('src');
    ...
    imageLoading.style.display = 'none';
    imageGenerateBtn.disabled = false;
    imageResultContainer.innerHTML = '<div class="image-error">Error al generar: ' + (err.message || String(err)) + '</div>';
  }
}
...
function downloadPdfFromMarkdown(markdown, profileName) {
  // Convertir Markdown simple a HTML para impresión
...
<head>
<meta charset="UTF-8">
<title>Reporte de Diagnóstico - ${escapeHtml(profileName)}</title>
<style>
  * { box-sizing: border-box; }
  body { font-family: 'Inter', 'Segoe UI', sans-serif; color: #1a1a2e; padding: 40px; line-height: 1.7; max-width: 800px; margin: 0 auto; }
  h1 { font-size: 22px; color: #0f0f1a; border-bottom: 3px solid #e50914; padding-bottom: 10px; margin-bottom: 24px; }
...
<div class="pdf-header">
  ...
  <div class="pdf-date">${escapeHtml(profileName)} — ${dateStr}</div>
</div>
${html}
<div class="pdf-footer">Diagnóstico orientativo generado por DMZ. Verifica siempre con multímetro antes de desoldar.</div>
</body>
</html>`);
